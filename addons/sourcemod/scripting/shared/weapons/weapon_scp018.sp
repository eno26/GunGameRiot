...
//stolen from https://github.com/redsunservers/SCP-Secret-Fortress/blob/master/addons/sourcemod/scripting/scp_sf/scps/018.sp

static const char SCP18Model[] = "models/passtime/ball/passtime_ball.mdl";
...
public void SCP18_TryTouchTeleport(float mins[3], float maxs[3], float dest[3])
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (IsPointTouchingBox(scp18.Position, mins, maxs))
		{
			...
			{
				Custom_SDKCall_SetLocalOrigin(scp18.EntIndex, dest);
	...
	for (int i = count-1; i >= 0; i--)
	{
		...
		if (TR_DidHit())
		{
			...
			*/		
			...
			Custom_SDKCall_SetLocalOrigin(scp18.EntIndex, hitposition);
			Custom_SetAbsVelocity(scp18.EntIndex, reflection);	
		...
		else
		{
			...
			Custom_SDKCall_SetLocalOrigin(scp18.EntIndex, nextposition);
			Custom_SetAbsVelocity(scp18.EntIndex, scp18.Velocity);
		}
...
public void SCP18_Button(int client, int weapon, bool crit)
{
