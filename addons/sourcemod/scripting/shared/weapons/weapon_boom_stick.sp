...
public void Weapon_BigFuckingGun(int client, int weapon, bool crit, int slot)
{
	float Ratio = 1.0;
	...
	if(SoundRatio > 1.0)
		SoundRatio = 1.0;
	EmitSoundToAll("misc/halloween/spell_mirv_explode_primary.wav", client, SNDCHAN_STATIC, 80, _, SoundRatio, 75);
	EmitSoundToAll("misc/halloween/spell_mirv_explode_primary.wav", client, SNDCHAN_STATIC, 80, _, SoundRatio, 75);
	EmitSoundToAll("misc/halloween/spell_mirv_explode_primary.wav", client, SNDCHAN_STATIC, 80, _, SoundRatio, 75);
	float ShakeRatio = Ratio;
	if(ShakeRatio > 1.3)
		ShakeRatio = 1.3;
	Client_Shake(client, 0, 90.0 * ShakeRatio, 60.0 * ShakeRatio, 1.2 * ShakeRatio);
}